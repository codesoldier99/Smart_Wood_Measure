...
def inference_wood(depth_pic_name, pic_name):
    fout = ""
    ...
    model = init_detector(global_var.model_configfile, global_var.checkpoint_file, global_var.device)
    ratio_list = list()
    ratio = 0.0008800518873684316
    ids = 0
    paths = global_var.pic_filename
    volume_total = 0
    volume_total1 = 0
    ...
    if segm_result is not None:  # non empty
        ...
        for i in inds:
            ...
            # rb = max(ra, rb)
            # 读取深度值
            depth_XY_data = getPngPix(pngPath=global_var.depth_pic_filename, pixelX=x, pixelY=y)
            print(depth_XY_data)
            # depth_XY_data = 1
            # print(depth_XY_data)
            # 计算深度阈值范围内的结果
            ...
            if int(D) % 2 == 0:  # 偶数
                ...
                D = int(D) - 1
            if depth_XY_data > 0 and depth_XY_data <= Depth_threshold :
                volume = Volume_Calculation(global_var.tree_longl, D)
                volume_total = volume_total + volume
                # 预测的小数木材尺径转换为1位小数尺径
                D1 = round(D1, 1)
                volume1 = Volume_Calculation(global_var.tree_longl, D1)
                volume_total1 = volume_total1 + volume1

            # 打印预测尺径
            #   cv2.putText(img, "{}".format(D1), (int(x) - 18, int(y) + 8), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)      # 200万像素图片
            #   cv2.putText(img, "{}".format(D1), (int(x) - 23, int(y) + 14), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)        # 800万像素图片
            # 打印序号和尺径
            # if depth_XY_data >0 and depth_XY_data <=Depth_threshold:
             # mobile
        ...
        filedir_out = os.path.join(global_var.output_dir, filename + '_' + 'cv2_mea_p' + filetype)
        fout = filedir_out
        # filedir_out2 = os.path.join(global_var.output_dir, filename + '_' + 'cv2_mea_p' + filetype)
        cv2.imwrite(filedir_out, img)

        # 将预测尺径信息保存进Excel
        xlsx_out_path = os.path.join(global_var.output_dir, filename + '_' + 'mea_volume_c' + '.xls')
        Statistics2Table(jingjitongji_list, jingjitongji_list1, depth_list, xlsx_out_path, tree_type='杉木',
                         tree_long=global_var.tree_longl)
...
if __name__ == '__main__':
    ...
    global_var._init()
    # global_var.pic_filename = 'C:\\backup2022_12_5\\backup2022_12_5\\Qt-Camera\\pic\\20220803122712.png'
    # global_var.depth_pic_filename = 'C:\\backup2022_12_5\\backup2022_12_5\\Qt-Camera\\pic\\depth20220803122712.png'
    # global_var.output_dir = 'C:\\backup2022_12_5\\backup2022_12_5\\Qt-Camera\\result'
    inference_wood(global_var.depth_pic_filename,global_var.pic_filename)